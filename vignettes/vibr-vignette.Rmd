...
```{r second example part 2}
   ident2 <- precheck_identification(X=X, delta=0.1,
       Xdensity_learners=Xdensity_learners[c(1,2,3)],
       Xbinary_learners=Xbinary_learners, threshold=10,
       scale_continuous = TRUE)
   ident2
...
```{r second example part 3}
   ident3 <- precheck_identification(X=X, delta=0.02,
       Xdensity_learners=Xdensity_learners[c(1,2,3)],
       Xbinary_learners=Xbinary_learners, threshold=10,
       scale_continuous = TRUE)
   ident3
...
Now we can fit the model with our selected value of delta. 
...
```{r second example part 5}
    ...
    vi3 <- varimp(X=X, 
                 Y=Y, 
                 delta=0.02, 
                 Y_learners = c(Y_learners, sl3::Lrnr_randomForest$new()), # added random forest from sl3/randomForest packages
                 Xdensity_learners=Xdensity_learners[1:2], #dropped one from previous block
                 Xbinary_learners=Xbinary_learners,
