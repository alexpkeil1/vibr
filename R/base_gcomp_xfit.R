# plug-in cross fit estimator using g-computation

.varimp_gcomp_xfit <- function(X,
                             Y,
                             V=NULL,
  ...
  if(any(is.na(ests))){
    if(foldrepeats==1){
      stop("Error in fit resulted in missing weights for some variables (try using 'W' for potentially problematic variables)")
    } else{
...
.varimp_gcomp_xfit_boot <- function(X,
                                  Y,
                                  V=NULL,
                                  whichcols=seq_len(ncol(X)),
                                  delta=0.1,
