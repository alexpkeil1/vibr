...
#'
#' This learner assumes a mean model with homoscedastic errors: log(Y) ~ E(log(Y)|W) + epsilon. E(log(Y)|W) is fit using a glm,
...
#' }
#'
Lrnr_density_loggaussian <- R6Class(
  classname = "Lrnr_density_loggaussian",
  ...
  private = list(
    ...
    .train = function(task) {
      ...
      args$y <- log(outcome_type$format(task$Y))
