...
jointtest <- function(){
  ...
  #dat = dgm( n=1000, delta = 0.01, beta = c(1,-2,2,0,0,0), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0))
...
set.seed(1231)
(res1 <- analyze(1231321, n=200, B=2, delta = 0.1, beta = c(-0.98,0.9,0.01,0.39,0.45,-0.32,0.76,-0.76,0.19,0.8,-0.22,0.94), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0))

set.seed(1231)
(res2 <- analyze(1231321, n=200, B=20, delta = 0.1, beta = c(-0.98,0.9,0.01,0.39,0.45,-0.32,0.76,-0.76,0.19,0.8,-0.22,0.94), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0))
...
system.time(res1 <- analyze(1231321, n=200, B=20, delta = 0.1, beta = c(-0.98,0.9,0.01,0.39,0.45,-0.32,0.76,-0.76,0.19,0.8,-0.22,0.94), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0))

csvout <- "/Users/akeil/temp/vimp_check.csv"
write.table(t(res1), csvout, append = FALSE, row.names = FALSE, sep=",")

resL = future.apply::future_lapply(1:1000, analyze, outfile=csvout, n=500, B=20, delta = 0.1, beta = c(-0.98,0.9,0.01,0.39,0.45,-0.32,0.76,-0.76,0.19,0.8,-0.22,0.94), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0,
...
cipow <- function(res, root="TMLE", exp="x", type="cover"){
  ...
  dat <- dgm(n=100, delta=0.1,beta = c(-0.98,0.9,0.01,0.39,0.45,-0.32,0.76,-0.76,0.19,0.8,-0.22,0.94), degree=2, zk = c(-1.5,-.25,0.25), sigma=sigma=1.0))
  ztest <- seq(-10,10,.002)
  xtest <- rep(c(0,1), length.out=length(ztest))
  pxz <- dnorm(ztest,0,3)*dbinom(xtest, 1, .2)
  pxzd <- dnorm(ztest-0.1,0,3)*dbinom(xtest, 1, .2)
